  ...
  return (
    <div className={`min-h-screen flex flex-col font-sans transition-colors duration-300 ${dm ? 'bg-gray-900 text-white' : 'bg-gray-50/50 text-gray-900'}`}>
      <Navbar
        ...
        onOrdersClick={() => {
          if (!currentUser) { alert("👋 Please Sign In first!"); setCurrentScreen('auth'); return; }
      ...
      <main className="flex-grow">
        {currentScreen === 'shop' && (
          <HomeScreen
            dbProducts={dbProducts} darkMode={dm} wishlist={wishlist}
            ...
            onBuyNow={(prod) => {
              if (!currentUser) { alert("👋 Please Sign In first!"); setCurrentScreen('auth'); return; }
              setCheckoutItem(prod); setCurrentScreen('checkout');
            }}
          />
        )}
        ...
        {currentScreen === 'detail' && selectedProduct && (
          <ProductDetailScreen product={selectedProduct} darkMode={dm} onBack={() => setCurrentScreen('shop')}
            onAddToCart={(prod) => { setCart([...cart, prod]); alert("🛒 Item added to bag!"); }}
            onBuyNow={(prod) => { setCheckoutItem(prod); setCurrentScreen('checkout'); }}
          />
        )}
        {currentScreen === 'checkout' && <CheckoutScreen darkMode={dm} item={checkoutItem} onProceedToPayment={() => setCurrentScreen('payment')} />}
        {currentScreen === 'payment' && <PaymentScreen darkMode={dm} item={checkoutItem} cart={cart} onPaymentSuccess={() => { setCart([]); setCheckoutItem(null); setCurrentScreen('success'); }} />}
        {currentScreen === 'orders' && <OrdersDashboard darkMode={dm} onBackToShop={() => setCurrentScreen('shop')} />}
        ...
        <div className="fixed inset-0 z-50 flex justify-end">
          ...
          <div className={`relative w-full max-w-sm h-full flex flex-col shadow-2xl animate-slideIn ${dm ? 'bg-gray-900' : 'bg-white'}`}>
            ...
            <div className={`flex items-center justify-between p-5 border-b ${dm ? 'border-gray-700' : 'border-gray-100'}`}>
              <h2 className={`font-black text-base uppercase tracking-wider ${dm ? 'text-white' : 'text-gray-800'}`}>
                🛒 My Bag ({cart.length})
              </h2>
              <button onClick={() => setCartOpen(false)} className={`w-8 h-8 rounded-full flex items-center justify-center font-black text-lg transition ${dm ? 'bg-gray-700 text-gray-300 hover:bg-gray-600' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}>✕</button>
            </div>
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-3">
              {cart.length === 0 ? (
                <div className="flex flex-col items-center justify-center h-full text-center py-16">
                  ...
                  <p className={`font-bold text-sm ${dm ? 'text-gray-400' : 'text-gray-500'}`}>Your bag is empty!</p>
                  <button onClick={() => setCartOpen(false)} className="mt-4 bg-indigo-600 text-white text-xs font-black px-5 py-2 rounded-xl hover:bg-indigo-700 transition">
                    Continue Shopping
                  </button>
                </div>
              ) : (
                cart.map((item, idx) => (
                  <div key={idx} className={`flex items-center gap-3 p-3 border rounded-2xl ${dm ? 'bg-gray-800 border-gray-700' : 'bg-gray-50 border-gray-100'}`}>
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {item.selectedColor && <p className={`text-[10px] ${dm ? 'text-gray-400' : 'text-gray-400'}`}>Color: {item.selectedColor}</p>}
                      {item.selectedSize && item.selectedSize !== 'Standard' && <p className={`text-[10px] ${dm ? 'text-gray-400' : 'text-gray-400'}`}>Size: {item.selectedSize}</p>}
                      <p className="text-xs font-black text-indigo-600 mt-0.5">₹{Number(item.price).toLocaleString('en-IN')}</p>
                    </div>
                    <button onClick={() => removeFromCart(idx)} className="text-red-400 hover:text-red-600 font-black text-lg transition">✕</button>
                  </div>
                ))
              )}
            </div>
            ...
            {cart.length > 0 && (
              <div className={`p-4 border-t space-y-3 ${dm ? 'border-gray-700' : 'border-gray-100'}`}>
                <div className={`flex justify-between items-center p-3 rounded-xl ${dm ? 'bg-gray-800' : 'bg-gray-50'}`}>
                  <span className={`text-xs font-bold ${dm ? 'text-gray-300' : 'text-gray-600'}`}>Total:</span>
                  <span className="text-base font-black text-indigo-600">
                    ₹{cart.reduce((sum, item) => sum + Number(item.price), 0).toLocaleString('en-IN')}
                  </span>
                </div>
                <button
                  onClick={() => {
                    setCartOpen(false);
                    if (!currentUser) { alert("👋 Please Sign In first!"); setCurrentScreen('auth'); return; }
                    setCheckoutItem(cart[0]);
                    setCurrentScreen('checkout');
                  }}
                  className="w-full bg-indigo-600 hover:bg-indigo-700 text-white font-black py-3 rounded-2xl text-xs uppercase tracking-wider transition shadow-md"
                >
      ...
      {/* WhatsApp Button */}
      <a href="https://wa.me/919389720893?text=Hi! I'm interested in ordering from JadounHub 🛍️"
        target="_blank" rel="noopener noreferrer"
        className="fixed bottom-6 right-6 z-40 bg-green-500 hover:bg-green-600 text-white rounded-full w-14 h-14 flex items-center justify-center shadow-lg hover:shadow-xl transition-all duration-300 hover:scale-110"
      >
